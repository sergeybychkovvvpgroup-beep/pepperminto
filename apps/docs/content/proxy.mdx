...
## Pepperminto behind a reverse proxy
...
An example of this would be a helpdesk company running Pepperminto having it accessible to employee's through their domain name.
...
```
https://pepperminto.example.com -> https://pepperminto-api.example.com -> nginx -> Pepperminto docker
```
...
## Setting up Pepperminto

Now nginx is out the way and configured, we can now simply download the docker compose file from github:

```
wget https://raw.githubusercontent.com/nulldoubt/Pepperminto/master/docker-compose.yml
```
...
server {
    listen 80;
    listen [::]:80;
    server_name pepperminto.example.com;
...
server {
    listen 80;
    listen [::]:80;
    server_name pepperminto-api.example.com;
    add_header Strict-Transport-Security "max-age=15552000; includeSubDomains" always;
...
You should now be able to see Pepperminto running on your choosen subdomain.
